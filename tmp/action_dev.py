...
from great_expectations.core.batch_spec import (
    ...
    SqlAlchemyDatasourceBatchSpec, RuntimeDataBatchSpec,
)
...
class DataHubValidationAction(ValidationAction):
    ...
    def get_dataset_partitions(self, batch_identifier, data_asset):
        ...
        if isinstance(data_asset, Validator) and (isinstance(
        ...
        ) or isinstance(data_asset.execution_engine, SparkDFExecutionEngine)):
            # For spark, {'data_asset_name': 'hadoop_catalog.sdm.s_tf_sports_t_order', 'batch_data': 'SparkDataFrame'}
            ge_batch_spec = data_asset.active_batch_spec
            partitionSpec = None
            # For spark, {'data_asset_name': 'hadoop_catalog.sdm.s_tf_sports_t_order', 'datasource_name': 'spark_iceberg_datasource'}
            batchSpecProperties = {
                "data_asset_name": str(
            ...
            sqlalchemy_uri = None
            # Spark engine has no engine attribute, so sqlalchemy_uri is None
            if hasattr(data_asset.execution_engine, "engine"):
            ...
            # For RuntimeDataRequest of Spark DataFrame
            elif isinstance(ge_batch_spec, RuntimeDataBatchSpec) and ge_batch_spec.get("batch_data") == "SparkDataFrame":
                schema = data_asset.active_batch_definition.batch_identifiers.get("datahub_schema")
                dataset = data_asset.active_batch_definition.batch_identifiers.get("datahub_dataset")
